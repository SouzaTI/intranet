...
// Cabeçalho robusto + fallback A:R + sem bloqueio automático por duplicidade.
...
function impMapaPadraoPosicional(): array {
    // Fallback para o layout oficial da planilha enviada (A:R).
    // Evita depender de acentos/locale do servidor para localizar "RAZÃO SOCIAL".
    return [
        'fornecedor' => 0,
        'nome_fantasia' => 1,
        'contato' => 2,
        ...
        'data_inicio' => 9,
        'comunicado_60_dias' => 10,
        'data_vencimento' => 11,
        'renovacao_automatica' => 12,
        'aviso_previo' => 13,
        'multa' => 14,
        'clausula_tecnica' => 15,
        'empresa' => 16,
...
function impMapearCabecalhos(array $cabecalhos): array {
    $aliases = [
        ...
        'COD SISTEMA' => 'codigo_sistema',
        'SERVICO' => 'servico_objeto',
        'OBJETO SERVICO' => 'servico_objeto',
        'CNPJ' => 'cnpj',
        'VALOR' => 'valor_planilha',
        'PRAZO' => 'prazo_planilha',
        'QUANT PAGTOS' => 'quantidade_pagamentos',
        'QUANT PAGAMENTO' => 'quantidade_pagamentos',
        'QUANT PAGAMENTOS' => 'quantidade_pagamentos',
        'QUANTIDADE PAGTOS' => 'quantidade_pagamentos',
        'QUANTIDADE PAGAMENTOS' => 'quantidade_pagamentos',
        'INICIO' => 'data_inicio',
        ...
        'DEPTO CONTRATANTE' => 'departamento',
        'SETOR CONTRATANTE' => 'departamento',
    ];
    ...
    foreach ($cabecalhos as $idx => $cabecalho) {
        ...
        if (isset($aliases[$chave])) {
            $mapa[$aliases[$chave]] = $idx;
            continue;
        }

        if (!isset($mapa['fornecedor'])
            && str_contains($chave, 'SOCIAL')
            && (str_contains($chave, 'RAZAO') || str_contains($chave, 'RAZ'))) {
            $mapa['fornecedor'] = $idx;
        } elseif (!isset($mapa['departamento'])
            && str_contains($chave, 'CONTRATANTE')
            && (str_contains($chave, 'DEPART') || str_contains($chave, 'DEPTO') || str_contains($chave, 'SETOR'))) {
            $mapa['departamento'] = $idx;
        } elseif (!isset($mapa['empresa'])
            && ($chave === 'EMPRESA' || str_contains($chave, 'EMPRESA CONTRATANTE'))) {
            $mapa['empresa'] = $idx;
        } elseif (!isset($mapa['cnpj']) && str_contains($chave, 'CNPJ')) {
            $mapa['cnpj'] = $idx;
        } elseif (!isset($mapa['servico_objeto']) && str_contains($chave, 'SERV')) {
            $mapa['servico_objeto'] = $idx;
        }
    }
...
function impLocalizarCabecalho(array $linhas): array {
    $limite = min(15, count($linhas));
    $melhorIndice = 0;
    $melhorMapa = [];
    $melhorPontuacao = -1;
    ...
    for ($i = 0; $i < $limite; $i++) {
        $linha = $linhas[$i] ?? [];
        $mapa = impMapearCabecalhos($linha);
        $pontuacao = count($mapa);
        ...
        if (isset($mapa['fornecedor'], $mapa['empresa'], $mapa['departamento'])) {
            return [$i, $mapa, false];
        }
    }

    // Fallback definitivo: se a linha tem o layout A:R, usa posição fixa.
    // Na planilha enviada: A=Razão Social, Q=Empresa, R=Departamento.
    for ($i = 0; $i < $limite; $i++) {
        $linha = $linhas[$i] ?? [];
        $naoVazios = array_filter($linha, fn($v) => $v !== null && trim((string) $v) !== '');
        if (count($linha) >= 18 && count($naoVazios) >= 5) {
            return [$i, impMapaPadraoPosicional(), true];
        }
    }

    return [$melhorIndice, $melhorMapa, false];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['acao'] ?? '') === 'analisar_planilha') {
    try {
        ...
        $_SESSION['importacao_contratos_mapa_posicional'] = $usouMapaPosicional;
    } catch (Throwable $e) {
...
$usou_mapa_posicional = !empty($_SESSION['importacao_contratos_mapa_posicional']);
$total = count($preview);
$total_ajustes = count(array_filter($preview, fn($r) => empty($r['setor_encontrado'])));
$total_prontos = $total - $total_ajustes;
    ...
    <div class="max-w-[1600px] mx-auto">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-7">
            <div>
                ...
                <div class="flex flex-wrap items-center gap-3"><h2 class="text-3xl font-black text-navy-900 tracking-tight uppercase italic">Importar Contratos</h2><span class="inline-flex items-center rounded-full bg-blue-100 text-blue-700 px-3 py-1 text-[10px] font-black uppercase tracking-wide">V3 • 20/08/2026</span></div>
                <p class="text-slate-500 font-medium mt-1">Carga inicial por planilha XLSX, com correção dos departamentos antes da gravação.</p><p class="text-xs font-black text-emerald-700 mt-1">Sem bloqueio automático por duplicidade: todas as linhas selecionadas serão importadas.</p>
            </div>
        </div>
        ...
        <section class="mb-5 grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div class="bg-emerald-50 border border-emerald-100 rounded-2xl p-4 shadow-sm"><p class="text-[10px] font-black uppercase text-emerald-600">Prontas para importar</p><p class="text-2xl font-black text-emerald-700"><?php echo max(0, $total_prontos); ?></p></div>
        ...
        <div class="bg-blue-50 border border-blue-100 rounded-2xl p-4 mb-4 text-sm text-blue-800">
            <strong><?php echo htmlspecialchars($nome_arquivo); ?></strong> analisada. <?php if ($usou_mapa_posicional): ?><strong>Fallback A:R ativado.</strong> <?php endif; ?>Quando um departamento não existir na matriz, escolha o correto na lista. Ao corrigir um nome, o mesmo ajuste é aplicado automaticamente às outras linhas com o mesmo departamento original.
        </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse min-w-[1450px]">
                        ...
                        <tbody class="divide-y divide-slate-100 text-xs">
                            ...
                            ?>
                            <tr class="<?php echo !$setorOk ? 'bg-amber-50/50' : 'hover:bg-blue-50/40'; ?>">
